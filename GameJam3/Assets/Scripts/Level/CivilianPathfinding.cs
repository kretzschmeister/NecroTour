...
public class CivilianPathfinding : AIBase
{
    ...
    GameObject player, safePlace, survivor;
    public GameObject defendPoint;
    // Afstand tussen civilian en bus
    Vector2 safePlaceRange;
    float distanceSafePlace;
    // Hoe groot de afstand moet zijn om naar de bus te gaan ipv de player
    public float range;
    ...
    void ChooseTarget()
    {
        ...
        for (int i = 0; i < (PathfindingWaypoints.pathfindingWaypoints.Length - (PathfindingWaypoints.points + 1)); i++)
        {
    ...
    Transform GetClosestWaypoint(Transform[] waypoints)
    {
        ...
        float closestDistance = Mathf.Infinity;
        for (int i = 0; i < (waypoints.Length - PathfindingWaypoints.points); i++)
        {
